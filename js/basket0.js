...
function renderProductsBasket(arr) {
    ...
    arr.forEach(card => {
        ...
        <div class="cart__product" data-product-id="${id}">
            <div class="cart__img">
                <img src="./images/${img[1]}" alt="${title}">
            </div>
        </div>
        `;
...
//             body: fileFormData
//         }).then(response => response.json())
//           .then(data => alert("Ваш заказ оформлен, скоро свяжемся с вами! С уважением, ECO DRIVE!"))
...
document.querySelector('#applicationForm').addEventListener('submit', function(e) {
    ...
    .then(res => res.text())
    .then(res => {
        alert("Ваш заказ сохранён в Google Таблицу! С уважением, ECO DRIVE!");
